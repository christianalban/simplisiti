...
const emit = defineEmits(['update:code', 'update:variables', 'update:name', 'save']);
...
<template>
    <div class="flex flex-col gap-4">
        <div class="flex justify-end">
            <button @click="save" type="button" class="button">{{ $t('buttons.save') }}</button>
        </div>
            ...
            <div>
                ...
                <div class="flex flex-col gap-2 bg-gray-100 p-2 rounded-lg">
                    <div v-for="(variable, index) of variables" class="flex form-group">
                        <select class="input" v-model="variable.type">
                        ...
                        <button type="button" class="button max-w-min" @click="addVariable" v-if="index === variables.length - 1">
                            <fa-icon icon="plus" />
                        </button>
                        <button type="button" class="button max-w-min" @click="removeVariable(index)" v-else>
                            <fa-icon icon="minus" />
                        </button>
                        <!-- buttons add and remove -->
                    </div>
